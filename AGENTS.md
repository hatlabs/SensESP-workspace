...
- `SensESP/NMEA0183 @ ^3.2.1` -- NMEA 0183 sentence parsing and the `NMEA0183IO` reader; see "Build Patterns"
...
**NMEA 0183 reading on the main loop.** Read the serial stream from the ReactESP event loop, not from a separate FreeRTOS task. The ESP32-C3 is single-core, so a reader task buys no parallelism and adds a cross-task propagation hazard between the parser and its consumers. Two idioms are in use: SensESP's `StreamLineProducer` feeding a `Filter<String>` (sentence-type gate) and `NMEA0183Parser`, as in `HALSER-ais-interface`, pinned `SensESP/NMEA0183 @ ^3.2.1`; or the NMEA0183 library's `NMEA0183IO`, pinned `SensESP/NMEA0183 @ ^3.2.0`, as in `HALSER-wind-interface`, hurma-wind-interface and gnss-rtk-compass. v3.2.0 was released on 2026-09-11 and is the first tag carrying `NMEA0183IO`; the git-main pin it needed before that is obsolete. v3.2.0 also changed `ParseInt(int*)` to `ParseInt(int32_t*)`, which breaks callers that pass an `int*`, such as `matsutec_ha102_parser.h` in the AIS projects; v3.2.1 restored the old signature, so pin new projects `^3.2.1`. Since v3.2.0 `NMEA0183IOTask` is a deprecated alias for `NMEA0183IO`, so older code that names it reads on the event loop once its pin resolves to v3.2.0 or later; write `NMEA0183IO` in new code. Because bytes now drain only when the loop ticks, give the UART more slack than the 256-byte default before `begin()`: at 38400 bit/s the default buffer plus the 128-byte hardware FIFO holds about 100 ms of AIS traffic, and `Serial1.setRxBufferSize(1024)`, the value the examples use, holds about 270 ms.
